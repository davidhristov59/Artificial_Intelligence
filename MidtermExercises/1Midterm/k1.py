...
class GhostOnSkates(Problem):
    ...
    def successor(self, state):
        ...
        #desno
        for i in range(3):
            new_state_x  = (ghost_x + i + 1 , ghost_y)

            if self.check_valid(new_state_x, self.walls, self.n):
                successors[f'Desno {i + 1}'] = new_state_x

        #gore
            new_state_y = (ghost_x, ghost_y + i + 1)

            if self.check_valid(new_state_y , self.walls, self.n):
                successors[f'Gore {i + 1}'] = new_state_y

        return successors

    #2 nacin
    def successors2(self, state):

        successors = dict()

        moves = ["Gore 1", "Gore 2", "Gore 3",
                        "Desno 1", "Desno 2", "Desno 3"]

        moves_coords = [(0 , 1), (0, 2), (0, 3),
                                        (1, 0), (2, 0), (3, 0)]

        for move, coord in zip(moves, moves_coords):
            new_ghost = (state[0] + coord[0], state[1] + coord[1])

            if self.check_valid(new_ghost, self.walls, self.n):
                successors[move] = new_ghost
    ...
    def h(self, node):
        ...
        ghost_x_pos , ghost_y_pos = node.state[0], node.state[1]
        pacman_x_pos, pacman_y_pos = self.goal[0], self.goal[1]

        return (abs(ghost_x_pos - pacman_x_pos) +
                abs(ghost_y_pos - pacman_y_pos)) / 3 # delime so 3 bidejki moze 3 pati najveke da se dvizi
